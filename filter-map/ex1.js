...
En utilisant filter puis map, la fonction doit renvoyer les noms des Jedis,
 c'est-à-dire les personnages du côté lumineux (pour ceux qui auraient passé
   les 40 dernières années dans une caverne en Lozère !)
...
function getJedisNames(characters) {
  ...
  const jedi = characters.filter(function(person) {return person.side === "light"; }).map(function(person) {
    return person.name;
  });
  return jedi;
}
